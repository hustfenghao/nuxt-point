<template>
  <div id="app">
    <h2>タスク</h2>
    <div>
      <input type="text" v-model="newTodoName">名前</input>
      <input type="text" v-model="newTodoName">メール</input>
      <input type="text" v-model="newTodoName">クーポン</input>
...
export default {
  ...
  created: function() {
    ...
    this.todosRef.on('value', function(snapshot) {
      ...
      console.log(_this.todos) // データに変化が起きたときに再取得する
    });
  },
  computed: {
